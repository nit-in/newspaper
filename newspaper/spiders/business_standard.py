    ...
    def parse(self, response):
        ...
        for response_link in response_links:
            ...
            try: 
                ...
                date = datetime.strptime(date,"%Y%m%d").strftime("%d-%b-%Y") #in format dd-mmm-yyyy
                #get article no for printing page as business-standard has a print page
                business_standard_link = config.BUSINESS_STANDARD_PRINT + str(article[-19:-5])
                article_name = article[:-20]
                self.tag = self.tag.replace("%20","_")
                self.tag = self.tag.strip()
                self.tag = self.tag.title()
                mpdf = make_pdf(str(self.name),str(business_standard_link),str(date),str(self.tag),str(article_name))
                mpdf.print()
